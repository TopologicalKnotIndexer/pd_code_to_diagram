...
if __name__ == "__main__":
    for filename in os.listdir(PD_CODE):
        ...
        out1_filepath = os.path.join(COORD3D, filename)
        out2_filepath = os.path.join(DIAGRAM, filename)

        # 输出一个三维结构
        run_with_redirect([os.path.join(DIRNOW, "..", "bin", "a.exe")], inp_filepath, out1_filepath)

        # 输出一个图像
        run_with_redirect([os.path.join(DIRNOW, "..", "bin", "a.exe"), "-d"], inp_filepath, out2_filepath)
